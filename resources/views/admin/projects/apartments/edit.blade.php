...
@section('content')
<div class="mb-3">
    <a href="{{ route('admin.projects.apartments.show', [$project, $apartment]) }}" class="btn btn-sm btn-outline-secondary">← К карточке квартиры</a>
</div>
<h1 class="h4 mb-4">Изменить квартиру № {{ $apartment->apartment_number }}</h1>

<form method="post" action="{{ route('admin.projects.apartments.update', [$project, $apartment]) }}">
    @csrf
    @method('PUT')
    <div class="row g-3">
        <div class="col-md-6">
    ...
    <div class="mt-3">
        ...
        <a href="{{ route('admin.projects.apartments.show', [$project, $apartment]) }}" class="btn btn-secondary">Отмена</a>
    </div>
</form>
@endsection
