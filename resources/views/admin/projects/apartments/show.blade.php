...
<div class="mb-3">
    <a href="{{ route('admin.projects.show', $project) }}" class="btn btn-sm btn-outline-secondary">← К проекту «{{ $project->name }}»</a>
</div>
        ...
        <div class="card">
            <div class="card-body">
                ...
                @if($apartment->layout_photo_url)
                    ...
                    <form method="post" action="{{ route('admin.projects.apartments.layout-photo', [$project, $apartment]) }}" enctype="multipart/form-data" class="mt-2">
                        @csrf
                ...
                @else
                    <form method="post" action="{{ route('admin.projects.apartments.layout-photo', [$project, $apartment]) }}" enctype="multipart/form-data">
                        @csrf
    ...
    <div class="col-lg-8">
        <div class="card mb-4">
            <div class="card-body">
                ...
                <hr>
                <a href="{{ route('admin.projects.apartments.edit', [$project, $apartment]) }}" class="btn btn-outline-primary btn-sm">Изменить</a>
                <form method="post" action="{{ route('admin.projects.apartments.destroy', [$project, $apartment]) }}" class="d-inline" onsubmit="return confirm('Удалить карточку квартиры?');">
                    @csrf
                    @method('DELETE')
