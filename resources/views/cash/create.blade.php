...
@section('title', 'Новый приход / расход')
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <select name="store_id" class="form-select" required>
                        @foreach($stores as $s)
                            <option value="{{ $s->id }}" {{ (old('store_id', $storeId) == $s->id) ? 'selected' : '' }}>{{ $s->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-6">
                    ...
                    <select name="operation_type_id" id="operation_type_id" class="form-select" required>
                        <optgroup label="Приход">
                            @foreach($incomeTypes as $t)
                                <option value="{{ $t->id }}" {{ old('operation_type_id') == $t->id ? 'selected' : '' }}>{{ $t->name }}</option>
                            @endforeach
                        </optgroup>
                ...
                <div class="col-12" id="target_store_wrap" style="{{ (old('operation_type_id') == $transferType?->id) ? '' : 'display:none' }}">
                    <label class="form-label">Касса назначения *</label>
                    <select name="target_store_id" id="target_store_id" class="form-select">
                        <option value="">— Выберите кассу</option>
                        @foreach($stores as $s)
                            <option value="{{ $s->id }}" {{ old('target_store_id') == $s->id ? 'selected' : '' }}>{{ $s->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <input type="number" name="amount" class="form-control" step="0.01" min="0.01" value="{{ old('amount') }}" placeholder="0.00" required>
                </div>
                <div class="col-12">
                    <label class="form-label">Комментарий</label>
                    <textarea name="comment" class="form-control" rows="2" placeholder="Основание, примечание">{{ old('comment') }}</textarea>
                </div>
            </div>
        </div>
    </div>
    <button type="submit" class="btn btn-primary">Создать документ</button>
    <a href="{{ route('cash.index') }}" class="btn btn-secondary">Отмена</a>
</form>
@push('scripts')
<script>
(function() {
