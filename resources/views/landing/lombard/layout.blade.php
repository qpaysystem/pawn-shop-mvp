...
<head>
    ...
    <title>@yield('title', 'Главная') — {{ config('services.lombard.name', 'Капитал') }}</title>
    ...
    <header class="header-capital">
        <nav class="navbar navbar-expand-lg navbar-dark py-0">
            <div class="container-capital d-flex flex-wrap align-items-center justify-content-between w-100">
                <a class="navbar-brand" href="{{ route('home') }}">{{ config('services.lombard.name', 'Капитал') }}</a>
                ...
                <div class="collapse navbar-collapse" id="navbarCapital">
                    <ul class="navbar-nav ms-auto align-items-lg-center">
                        ...
                        <li class="nav-item"><a class="nav-link" href="{{ route('landing.lombard') }}">Ломбард</a></li>
                        <li class="nav-item"><a class="nav-link" href="{{ route('landing.buy') }}">Покупка</a></li>
                        <li class="nav-item"><a class="nav-link" href="{{ route('landing.about') }}">О компании</a></li>
                        <li class="nav-item"><a class="nav-link" href="{{ route('landing.contacts') }}">Контакты</a></li>
                        <li class="nav-item"><a class="nav-link" href="{{ route('landing.catalog') }}">Каталог</a></li>
                        <li class="nav-item"><a class="nav-link" href="{{ route('kb.index') }}">База знаний</a></li>
                        <li class="nav-item ms-lg-3">
                            <a class="btn" href="{{ route('login') }}">Вход для сотрудников</a>
                        </li>
                    </ul>
                </div>
            </div>
        ...
        <div class="container-capital">
            <div class="row align-items-center">
                <div class="col-md-6 text-center text-md-start mb-3 mb-md-0">
                    <strong>{{ config('services.lombard.name', 'Капитал') }}</strong>
                </div>
                <div class="col-md-6 text-center text-md-end">
                    <a href="tel:{{ preg_replace('/[^0-9+]/', '', config('services.lombard.phone')) }}">{{ config('services.lombard.phone') }}</a>
                    <span class="mx-2">|</span>
                    <a class="btn btn_blue" href="{{ route('login') }}" style="padding: 8px 20px; font-size: 14px;">Вход для сотрудников</a>
                </div>
            </div>
        </div>
    </footer>
