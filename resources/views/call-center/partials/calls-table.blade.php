<div class="d-flex flex-wrap gap-2 mb-3 align-items-center">
    ...
    <form method="post" action="{{ route('call-center.sync-mts-recordings') }}" class="d-inline" title="Скачать записи, расшифровать все без текста (МТС STT / Whisper) и отправить в agent-teams.home">
        @csrf
        <button type="submit" class="btn btn-outline-secondary btn-sm"><i class="bi bi-mic"></i> Расшифровать все (90 дн.)</button>
    </form>
...
@if($contacts->isEmpty())
    <p class="text-muted">Нет звонков за выбранный период.</p>
@else
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                ...
                @endif
                @if($c->contact_phone)
                    <br><small class="text-muted" title="Телефон абонента">{{ $c->contact_phone }}</small>
                @endif
            </td>
            <td>
                @if($c->line_phone)
                    <small class="text-muted d-block">{{ $c->line_phone_caption }}</small>
                    {{ $c->line_phone }}
                @else
                    <span class="text-muted">—</span>
                @endif
