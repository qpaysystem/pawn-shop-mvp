...
<form method="post" action="{{ route('users.update', $user) }}">@csrf @method('PUT')
    ...
    <div class="mb-3"><label class="form-label">Роль *</label><select name="role" class="form-select" id="user_role">@foreach(['super-admin' => 'Супер-админ', 'manager' => 'Управляющий', 'appraiser' => 'Оценщик', 'cashier' => 'Кассир', 'storekeeper' => 'Кладовщик', 'contact-center' => 'Контакт-центр'] as $v => $l)<option value="{{ $v }}" {{ old('role', $user->role) === $v ? 'selected' : '' }}>{{ $l }}</option>@endforeach</select></div>
...
document.getElementById('user_role').addEventListener('change', function() {
    const hideStore = this.value === 'super-admin' || this.value === 'contact-center';
    document.getElementById('user_store_block').style.display = hideStore ? 'none' : 'block';
});
{
    const role = document.getElementById('user_role').value;
    const hideStore = role === 'super-admin' || role === 'contact-center';
    if (hideStore) document.getElementById('user_store_block').style.display = 'none';
}
</script>
@endpush
@endsection
