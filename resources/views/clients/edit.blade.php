...
<form method="post" action="{{ route('clients.update', $client) }}" id="client-form">@csrf @method('PUT')
    <div class="mb-4">
        <label class="form-label">Статус клиента</label>
        <div class="d-flex gap-3">
            <label class="form-check">
                <input type="radio" name="client_type" value="individual" class="form-check-input" {{ old('client_type', $client->client_type ?? 'individual') === 'individual' ? 'checked' : '' }}>
                <span class="form-check-label">Физическое лицо</span>
            </label>
            <label class="form-check">
                <input type="radio" name="client_type" value="legal" class="form-check-input" {{ old('client_type', $client->client_type ?? 'individual') === 'legal' ? 'checked' : '' }}>
    ...
    <div id="block-individual" class="mb-3">
        <div class="row mb-3">
            <div class="col-md-4"><label class="form-label">Фамилия *</label><input type="text" name="last_name" class="form-control" value="{{ old('last_name', $client->last_name) }}"></div>
            <div class="col-md-4"><label class="form-label">Имя *</label><input type="text" name="first_name" class="form-control" value="{{ old('first_name', $client->first_name) }}"></div>
            <div class="col-md-4"><label class="form-label">Отчество</label><input type="text" name="patronymic" class="form-control" value="{{ old('patronymic', $client->patronymic) }}"></div>
        </div>
    </div>
    ...
    <div id="block-legal" class="mb-3" style="display: none;">
        <div class="card card-body mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="legal_name" class="form-control" value="{{ old('legal_name', $client->legal_name) }}" placeholder="ООО «Ромашка»">
            </div>
            <div class="row">
                <div class="col-md-4 mb-3">
                    <label class="form-label">ИНН</label>
                    <input type="text" name="inn" class="form-control" value="{{ old('inn', $client->inn) }}" maxlength="12" placeholder="10 или 12 цифр">
                </div>
                <div class="col-md-4 mb-3">
                    <label class="form-label">КПП</label>
                    <input type="text" name="kpp" class="form-control" value="{{ old('kpp', $client->kpp) }}" maxlength="9" placeholder="9 цифр">
                </div>
            </div>
            <div class="mb-0">
                <label class="form-label">Юридический адрес</label>
                <textarea name="legal_address" class="form-control" rows="2" placeholder="Индекс, город, улица, дом">{{ old('legal_address', $client->legal_address) }}</textarea>
            </div>
        </div>
        <p class="small text-muted">Контактное лицо (ФИО) можно указать в полях выше или в заметках.</p>
    </div>

    <div class="mb-3"><label class="form-label">Телефон *</label><input type="text" name="phone" class="form-control" value="{{ old('phone', $client->phone) }}" required></div>
    <div class="mb-3"><label class="form-label">Email</label><input type="email" name="email" class="form-control" value="{{ old('email', $client->email) }}"></div>
