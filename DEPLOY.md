...
Скрипт `deploy.sh` сам выполнит: установку зависимостей (`composer install --no-dev`), миграции, очистку и сборку кэша. Если на сервере нет `deploy.sh` или нужны только команды вручную — см. раздел **«Git на Timeweb»** → подраздел **«5. Обновление сайта после изменений в коде»** (вариант Б).
