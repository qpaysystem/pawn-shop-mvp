...
## Внесение изменений на боевой сервер (деплой через Git)

Если сайт уже работает и проект на сервере подключён к Git — обновления выкатываются так же, как в **client-management-crm**.
...
В каталоге проекта (например `~/pawn-shop-mvp`):

```bash
cd ~/pawn-shop-mvp
git pull origin main
# Если на хостинге по умолчанию старый PHP, укажите явно:
export PHP=/usr/bin/php
./deploy.sh
```

Скрипт `deploy.sh` сам выполнит: установку зависимостей (`composer install --no-dev`), миграции, очистку и сборку кэша.

**Проверка версии после деплоя:** если при «Загрузить из 1С» видите сообщение *«Не удалось получить данные из 1С (сервер недоступен или ошибка)...»* — на сервере всё ещё старая версия кода. Выполните `git pull origin main` и `./deploy.sh` ещё раз; затем `grep -n "устаревший код" app/Http/Controllers/ClientController.php` — должна быть строка с этим текстом. Если на сервере нет `deploy.sh` или нужны только команды вручную — см. раздел **«Git на Timeweb»** → подраздел **«5. Обновление сайта после изменений в коде»** (вариант Б).

### Одной строкой с локальной машины (по желанию)

Подставьте свой логин и хост:

```bash
git push origin main && ssh ЛОГИН@ХОСТ 'cd ~/pawn-shop-mvp && git pull origin main && ./deploy.sh'
```

После `git pull` не перезаписывайте `.env` на сервере — там остаются свои `APP_KEY`, `DB_*` и т.д.
...
- **Администратор:** логин `admin@example.com`, пароль `password`
- **Оценщик / товаровед** (приём, выкуп, касса): логин `appraiser@example.com`, пароль `password`  
  (создаётся сидером только если в БД есть хотя бы один магазин)

Эти данные создаются сидером на шаге 11.

**Если оценщик видит полное меню вместо упрощённого:** на сервере проверьте роль в БД. Должно быть ровно `appraiser`. Исправление (по SSH, в каталоге проекта):  
`/usr/bin/php artisan tinker --execute="\$u = App\Models\User::where('email','appraiser@example.com')->first(); if (\$u) { \$u->update(['role'=>'appraiser']); echo 'OK'; } else { echo 'User not found'; }"` После первого входа рекомендуется сменить пароль в разделе «Профиль».
