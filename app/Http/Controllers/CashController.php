...
class CashController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if ($operationType->name === 'Перемещение между кассами' && ! empty($validated['target_store_id'])) {
            $ledger->postTransfer($amount, $entryDate, (int) $validated['store_id'], (int) $validated['target_store_id'], 'cash_document', $doc->id, $comment);
        } elseif ($operationType->isIncome()) {
            $ledger->postSingle(Account::CODE_CASH, $amount, 0, $entryDate, (int) $validated['store_id'], 'cash_document', $doc->id, $comment);
        } else {
            $ledger->postSingle(Account::CODE_CASH, 0, $amount, $entryDate, (int) $validated['store_id'], 'cash_document', $doc->id, $comment);
        }
