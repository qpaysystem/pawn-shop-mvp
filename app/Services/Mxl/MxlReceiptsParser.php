...
/** Парсер отчёта 1С MOXCEL «Поступления» (скупка + залог). */
class MxlReceiptsParser
{
    /**
     * @return list<array{
     *   date: ?Carbon,
    ...
    private function parseChunk(string $chunk): ?array
    {
        ...
        if (! preg_match('/\{"#","((?:Скупка ценностей|Операция по залогу)[^"]+)"\}/u', $chunk, $docMatch)) {
            return null;
        }
        ...
        $isPurchase = str_starts_with($document, 'Скупка');
